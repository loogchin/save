...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'no':
				defaultCamZoom = 0.7;
				var no = new FlxSprite(-430,-180);
				no.loadGraphic(Paths.image('bg','shared'));
				no.setGraphicSize(Std.int(no.width * 1.2));
				no.updateHitbox();
				add(no);
		}
		...
		if(curSong == 'warmth-chill')
			{
				o = new FlxSprite();
				o.loadGraphic(Paths.image('o','shared'));
				o.cameras = [camHUD];
				o.alpha = 0;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if (!SONG.notes[Math.floor(curStep / 16)].mustHitSection && cameraTwn == null)
				{
					var pluscamera:Array<Int> = [150,-100];

					if (SONG.notes[Std.int(curStep / 16)].gfSection){
						pluscamera = [0,0];
					}

					var xx = (dad.getMidpoint().x + pluscamera[0]) + dad.cameraPosition[0];
					var yy = (dad.getMidpoint().y + pluscamera[1]) + dad.cameraPosition[1];
					var ofs = 60;

					switch (Std.int(Math.abs(note.noteData)))
					{
						case 0:
							triggerEventNote('Camera Follow Pos',Std.string(Std.int(xx-ofs)),Std.string(Std.int(yy)));
						case 1:
							triggerEventNote('Camera Follow Pos',Std.string(Std.int(xx)),Std.string(Std.int(yy+ofs)));
						case 2:
							triggerEventNote('Camera Follow Pos',Std.string(Std.int(xx)),Std.string(Std.int(yy-ofs)));
						case 3:
							triggerEventNote('Camera Follow Pos',Std.string(Std.int(xx+ofs)),Std.string(Std.int(yy)));
			...
			if(curSong == 'warmth-chill' && oBool)
				{
					FlxTween.tween(o, {alpha: 1}, 0.5);
					o.color = FlxColor.fromRGB(46,110,72);
				}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if (SONG.notes[Math.floor(curStep / 16)].mustHitSection && cameraTwn == null)
					{
						var xx = (boyfriend.getMidpoint().x - 100) - boyfriend.cameraPosition[0]; 
						var yy = (boyfriend.getMidpoint().y - 100) + boyfriend.cameraPosition[1]; 
						var ofs:Int = 60;
				...
				{
					FlxTween.tween(o, {alpha: 1}, 0.5);
					o.color = FlxColor.fromRGB(49,176,209);
				}
	...
	var beatcolor:Bool = false;
	var hitcolor:Bool = false;
	...
	override function beatHit()
	{
		...
		if(curSong == 'warmth-chill' && camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % gfSpeed == 0 && curBeat <= 80)
		{
			FlxG.camera.zoom += 0.03;
			camHUD.zoom += 0.03;
		}
		if(curSong == 'warmth-chill' && camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % gfSpeed == 0 && curBeat >= 109)
			{
				FlxG.camera.zoom += 0.03;
				camHUD.zoom += 0.03;
			}
